...
<?php
    if ($skin->errors) {
        foreach ($skin->errors as $error) {
            echo '<div class="pure-u-1 bg-error">' . $error . '</div>';
        }
    }
    if ($skin->messages) {
        foreach ($skin->messages as $message) {
...
<script type="text/javascript">
    ...
    img.src = 'https://skins.manicdigger.io/<?php echo $_SESSION['user_name']; ?>.png';
        ...
        renderer.render(scene, camera);
            ...
            <div class="centerbox-container">
                <div class="centerbox pure-g">
                    <h1 class="is-center pure-u-1">Your Skin</h1>
                    <!-- register form -->
                    <form class="pure-form" method="post" enctype="multipart/form-data" action="/skin.php" name="skinform">
                        <hr />
                        <fieldset>
                            <!-- the user name input field uses a HTML5 pattern check -->
                            <input type="hidden" value="blank" name="without_this_the_uploader_breaks_for_some_reason">
                            <input type="file" name="skin" class="skinupload">
                        </fieldset>
                        <hr />
                        <button class="pure-button bg-success pure-u-1 pure-u-md-2-3 pull-right" type="submit" name="upload" value="Change Skin">Change Skin</button>
                    </form>
                </div>
            </div>
<?php } ?>
