...
class User {
	...
	public static function getUser($username) {
		...
		$stmt = $mysqli->prepare("SELECT user_id, user_name, user_password_hash, registered_on, last_login FROM users WHERE UPPER(user_name) = UPPER(?)");
		if ($mysqli->errno) {
		...
		$stmt->bind_result($userId, $username, $passwordHash, $joinedDate, $lastLoginDate);
	...
	public static function createUser(User $user) {
		...
		$stmt = $mysqli->prepare("INSERT INTO users(user_name, user_password_hash, last_login) VALUES (?,?, NOW())");
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}
		
		//Bind parameters
		$stmt->bind_param("ss", $user->getUsername(), $user->getPasswordHash());
	...
	public static function updateUser(User $user) {
		...
		$stmt = $mysqli->prepare("UPDATE users SET user_password_hash = ? WHERE user_id = ?");
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}
		
		//Bind parameters
		$stmt->bind_param("si", $user->getPasswordHash(), $user->getEmail(), $user->getUserId());
