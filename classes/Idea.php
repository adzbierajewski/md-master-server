<?php
require_once("Utility.php");

class Idea {

	//Static Functions
	public static function updateServer(Server $idea) {
		if(Server::getServerByPrivateKey($idea->getPrivateKey()) === null) {
			return Server::createServer($idea);
		}

		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "UPDATE servers SET ";
		$sql .= "address = ?, port = ?, version = ?, name = ?, key_hash = ?, max_clients = ?, ";
		$sql .= "public = ?, password_protected = ?, allow_guests = ?, user_count = ?, ";
		$sql .= "user_list = ?, motd = ?, game_mode = ?, last_heartbeat_date = NOW() ";
		$sql .= "WHERE private_key = ?";


		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		//Bind parameters
		$stmt->bind_param("sissssississss",
			$idea->getAddress(),
			$idea->getPort(),
			$idea->getVersion(),
			$idea->getName(),
			$idea->getPublicKey(),
			$idea->getMaxClients(),
			$idea->getPublic(),
			$idea->getPasswordProtected(),
			$idea->getAllowGuests(),
			$idea->getUserCount(),
			$idea->getUserList(),
			$idea->getMotd(),
			$idea->getGameMode(),
			$idea->getPrivateKey());

		//Execute statement
		$stmt->execute();

		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->close();
		unset($stmt);

		return "Server updated succesfully.\nserver=" . $idea->getPublicKey();
	}

	//Note: this is private.
	private static function createServer(Server $idea) {
		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "INSERT INTO servers(";
		$sql .= "address, port, version, name, private_key, key_hash, max_clients, public, password_protected, ";
		$sql .= "allow_guests, user_count, user_list, motd, game_mode, last_heartbeat_date";
		$sql .= ") VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())";

		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		//Bind parameters
		$stmt->bind_param("sissssisssisss",
			$idea->getAddress(),
			$idea->getPort(),
			$idea->getVersion(),
			$idea->getName(),
			$idea->getPrivateKey(),
			$idea->getPublicKey(),
			$idea->getMaxClients(),
			$idea->getPublic(),
			$idea->getPasswordProtected(),
			$idea->getAllowGuests(),
			$idea->getUserCount(),
			$idea->getUserList(),
			$idea->getMotd(),
			$idea->getGameMode());

		//Execute statement
		$stmt->execute();

		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->close();
		unset($stmt);

		return "Server added successfully.\nserver=" . $idea->getPublicKey();
	}

	public static function getServerByPublicKey($keyHash) {
		$idea = new Idea();

		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "SELECT server_id, address, port, version, name, private_key, key_hash, ";
		$sql .= "max_clients, public, password_protected, allow_guests, user_count, ";
		$sql .= "user_list, motd, game_mode, last_heartbeat_date, created_date ";
		$sql .= "FROM servers WHERE key_hash = ?";

		//Prepare query to find server
		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_param("s", $keyHash);

		//Run query
		$stmt->execute();
		if ($mysqli->errno) {
			$stmt->close();
		    unset($stmt);
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_result($ideaId, $address, $port, $version, $name, $privateKey, $keyHash,
						   $maxClients, $public, $passwordProtected, $allowGuests, $userCount,
						   $userList, $motd, $gameMode, $lastHeartbeatDate, $createdDate);

		$result = $stmt->fetch();

		$stmt->close();
		unset($stmt);

		if(!$result) { //Not found or error.
			return null;
		}

		//Fill server object
		$idea->setServerId($ideaId);
		$idea->setAddress($address);
		$idea->setPort($port);
		$idea->setVersion($version);
		$idea->setName($name);
		$idea->setPrivateKey($privateKey);
		$idea->setPublicKey($keyHash);
		$idea->setMaxClients($maxClients);
		$idea->setPublic($public);
		$idea->setPasswordProtected($passwordProtected);
		$idea->setAllowGuests($allowGuests);
		$idea->setUserCount($userCount);
		$idea->setUserList($userList);
		$idea->setMotd($motd);
		$idea->setGameMode($gameMode);
		$idea->setLastHeartbeatDate($lastHeartbeatDate);
		$idea->setCreatedDate($createdDate);

		return $idea;
	}

	public static function getServerByPrivateKey($privateKey) {
		$idea = new Idea();

		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "SELECT server_id, address, port, version, name, private_key, key_hash, ";
		$sql .= "max_clients, public, password_protected, allow_guests, user_count, ";
		$sql .= "user_list, motd, game_mode, last_heartbeat_date, created_date ";
		$sql .= "FROM servers WHERE private_key = ?";

		//Prepare query to find server
		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_param("s", $privateKey);

		//Run query
		$stmt->execute();
		if ($mysqli->errno) {
			$stmt->close();
		    unset($stmt);
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_result($ideaId, $address, $port, $version, $name, $privateKey, $keyHash,
						   $maxClients, $public, $passwordProtected, $allowGuests, $userCount,
						   $userList, $motd, $gameMode, $lastHeartbeatDate, $createdDate);

		$result = $stmt->fetch();

		$stmt->close();
		unset($stmt);

		if(!$result) { //Not found or error.
			return null;
		}

		//Fill server object
		$idea->setServerId($ideaId);
		$idea->setAddress($address);
		$idea->setPort($port);
		$idea->setVersion($version);
		$idea->setName($name);
		$idea->setPrivateKey($privateKey);
		$idea->setPublicKey($keyHash);
		$idea->setMaxClients($maxClients);
		$idea->setPublic($public);
		$idea->setPasswordProtected($passwordProtected);
		$idea->setAllowGuests($allowGuests);
		$idea->setUserCount($userCount);
		$idea->setUserList($userList);
		$idea->setMotd($motd);
		$idea->setGameMode($gameMode);
		$idea->setLastHeartbeatDate($lastHeartbeatDate);
		$idea->setCreatedDate($createdDate);

		return $idea;
	}

	public static function getSingleIdea($id) {
		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "SELECT idea_id, name, last_heartbeat_date, created_date, creator, text FROM ideas WHERE idea_id = ?";
		//$sql .= "WHERE last_heartbeat_date > (NOW() - INTERVAL 8 HOUR) ";
		//$sql .= "ORDER BY last_heartbeat_date DESC";

		//Prepare query to find server
		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_param("i", $id);
		//Run query
		$stmt->execute();
		if ($mysqli->errno) {
			$stmt->close();
		    unset($stmt);
			trigger_error($mysqli->error,E_USER_ERROR);
		}
		$stmt->bind_result($ideaId, $Name, $lastHeartbeatDate, $createdDate, $Creator, $Text);

		$ideaList = array();
		while($stmt->fetch()) {
			$idea = new Idea();

			//Fill server object
			$idea->setIdeaId($ideaId);
			$idea->setName($Name);
			$idea->setLastHeartbeatDate($lastHeartbeatDate);
			$idea->setCreatedDate($createdDate);
			$idea->setCreator($Creator);
			$idea->set_Text($Text);

			$ideaList[] = $idea;
		}

		$stmt->close();
		unset($stmt);

		return $ideaList;
	}
	
	public static function getIdeaList() {
		//Grab connection object
		$mysqli = Utility::getSQLConnection();

		$sql =  "SELECT idea_id, name, last_heartbeat_date, created_date, creator, text FROM ideas ";
		//$sql .= "WHERE last_heartbeat_date > (NOW() - INTERVAL 8 HOUR) ";
		//$sql .= "ORDER BY last_heartbeat_date DESC";

		//Prepare query to find server
		$stmt = $mysqli->prepare($sql);
		if ($mysqli->errno) {
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		//Run query
		$stmt->execute();
		if ($mysqli->errno) {
			$stmt->close();
		    unset($stmt);
			trigger_error($mysqli->error,E_USER_ERROR);
		}

		$stmt->bind_result($ideaId, $Name, $lastHeartbeatDate, $createdDate, $Creator, $Text);

		$ideaList = array();
		while($stmt->fetch()) {
			$idea = new Idea();

			//Fill server object
			$idea->setIdeaId($ideaId);
			$idea->setName($Name);
			$idea->setLastHeartbeatDate($lastHeartbeatDate);
			$idea->setCreatedDate($createdDate);
			$idea->setCreator($Creator);
			$idea->set_Text($Text);

			$ideaList[] = $idea;
		}

		$stmt->close();
		unset($stmt);

		return $ideaList;
	}

	//Internal Data
	private $ideaId;
	private $Name;
	private $lastHeartbeatDate;
	private $createdDate;
	private $Creator;
	private $Text;


	//Getters/Setters
	public function setIdeaId($ideaId){
		$this->ideaId = $ideaId;
	}
	
	public function getIdeaId(){
		return $this->ideaId;
	}

	public function getName(){
		return $this->name;
	}

	public function setName($name){
		$this->name = $name;
	}
	
	public function get_Text(){
		return $this->text;
	}

	public function set_Text($text){
		$this->text = $text;
	}
	
	public function getLastHeartbeatDate(){
		return $this->lastHeartbeatDate;
	}

	public function setLastHeartbeatDate($lastHeartbeatDate){
		$this->lastHeartbeatDate = $lastHeartbeatDate;
	}
	
	public function getCreatedDate(){
		return $this->createdDate;
	}

	public function setCreatedDate($createdDate){
		$this->createdDate = $createdDate;
	}

	public function getCreator(){
		return $this->creator;
	}

	public function setCreator($creator){

		$this->creator = $creator;
	}

	public function getVersion(){
		return $this->version;
	}

	public function setVersion($version){
		if(strlen($version) > 32) {
			throw new Exception("Error: Version length exceeded. (Max 32)");
		}
		$this->version = $version;
	}

	public function getPrivateKey(){
		return $this->privateKey;
	}

	public function setPrivateKey($privateKey){
		if(strlen($privateKey) > 32) {
			throw new Exception("Error: Private key length exceeded. (Max 32)");
		}
		$this->keyHash = md5($privateKey . LIST_KEY);
		$this->privateKey = $privateKey;
	}

	public function getPublicKey(){
		return $this->keyHash;
	}

	public function setPublicKey($keyHash){
		if(strlen($keyHash) > 32) {
			throw new Exception("Error: Public key length exceeded. (Max 32)");
		}
		$this->keyHash = $keyHash;
	}

	public function getMaxClients(){
		return $this->maxClients;
	}

	public function setMaxClients($maxClients){
		$this->maxClients = intval($maxClients);
	}

	public function getPublic(){
		return $this->public;
	}

	public function setPublic($public){
		if(strlen($public) > 5) {
			throw new Exception("Error: Public value length exceeded. (Max 5)");
		}
		$this->public = $public;
	}

	public function getPasswordProtected(){
		if(strtoupper($this->passwordProtected) === "TRUE") {
			return "True";
		} else {
			return "False";
		}
	}

	public function setPasswordProtected($passwordProtected){
		if(strlen($passwordProtected) > 5) {
			throw new Exception("Error: Password protected value length exceeded. (Max 5)");
		}
		$this->passwordProtected = $passwordProtected;
	}

	public function getAllowGuests(){
		return $this->allowGuests;
	}

	public function setAllowGuests($allowGuests){
		if(strlen($allowGuests) > 5) {
			throw new Exception("Error: Allow guests value length exceeded. (Max 5)");
		}
		$this->allowGuests = $allowGuests;
	}

	public function getUserCount(){
		return $this->userCount;
	}

	public function setUserCount($userCount){
		$this->userCount = intval($userCount);
	}

	public function getUserList(){
		return $this->userList;
	}

	public function setUserList($userList){
		if(strlen($userList) > 1024) {
			$userList = substr($userList, 0, 1024);
		}
		$this->userList = $userList;
	}

	public function getMotd(){
		return $this->motd;
	}

	public function setMotd($motd){
		if(strlen($motd) > 1024) {
			$motd = substr($motd, 0, 1024);
		}
		$this->motd = $motd;
	}

	public function getGameMode(){
		return $this->gameMode;
	}

	public function setGameMode($gameMode){
		if(strlen($gameMode) > 32) {
			$gameMode = substr($gameMode, 0, 32);
		}
		$this->gameMode = $gameMode;
	}
}
